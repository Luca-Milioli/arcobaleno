...
func moved(moved_fruit: Fruit) -> void:
	...
	if new_visible:
		var last = get_child(TOTAL_SLOT - 1) as Slot
		original_positions[new_visible] = last.global_position + Vector2(get_theme_constant("separation") + last.size.x, 0)
...
func _animate_enter_or_exit(slot: Slot, enter: bool, duration: float = 0.8) -> void: # enter = true, exit = false
	...
	tween.set_parallel()
	tween.tween_property(slot, "modulate:a", end_modulate, duration).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
